...
#        results = (n_modules, n_automodules, n_rsttests, n_unittests, n_doctests)
        results = (n_modules, n_automodules, n_rsttests, n_doctests)    
        self.print_comparison(results)
    
        if self.is_dev_version:
            # + n_unit_errors
            if n_mod_errors + n_doc_errors + n_doc_errors == 0:
...
#         return (n_unittests, n_errors)
    ...
    def print_comparison (self, results):
        # , n_unittests
        (n_modules, n_automodules, n_rsttests, n_doctests) = results
...
#        print(f"Unit tests:         {n_unittests:4d} {values[3]:4d} {'**' if n_unittests != values[3] else ''}")
        print(f"Doctests:           {n_doctests:4d} {values[3]:4d} {'**' if n_doctests != values[3] else ''}")
    ...
    def save_results (self, results):
        ...
        with open('previous_results', 'w') as f:
            # , n_unittests
            print(' '.join(str(v) for v in (n_modules, n_automodules, n_rsttests, n_doctests)), file=f)
